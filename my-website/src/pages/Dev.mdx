...
- Reactivity: pages are responsive. A key hurdle here was learning that website should be build mobile-first, not browser-first.
- Dark mode toggle: the toggle is implemented using React's Context API.
- Deployment through Cloudflare Pages: documentation for both Cloudflare and Vite was spare and I had to fill in the gaps through trial and error.
- Safari hates favicons and being a modern browser generally.
...
- Favicons that adjust according to user's theme preference using `@media (prefers-color-scheme: dark)`.
